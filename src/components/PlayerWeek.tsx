...
import { joinEvent, leaveEvent, hasJoined, type EventWithResponses } from '../lib/events'
import { applyDefaultWeek, hasDefaultWeek, saveDefaultWeek } from '../lib/settings'
import type { MyTeam } from '../lib/teams'
import { friendlyError, roleLabel, type MemberWithProfile, type TeamSlot } from '../lib/types'
import type { WeekData } from '../lib/useWeekData'
import { dayShort, slotLabel, toDateKey, weekId, type WeekDay } from '../lib/week'
import SessionsList from './SessionsList'
import { Avatar, Button, DotRow, ErrorText, Spinner, useToast } from './ui'
...
export default function PlayerWeek({ team, userId, members, week }: Props) {
  const { days, slots, hours, setHours, hoursRef, events, setEvents, loaded, error, setError } = week
  ...
  const myHourCount = useMemo(() => days.reduce((n, d) => n + (hours[d.key]?.[userId]?.size ?? 0), 0), [days, hours, userId])
  ...
  async function toggle(day: WeekDay, slot: TeamSlot, daySlots: TeamSlot[]) {
    ...
    try {
      ...
      setError(null)
      toast(`Saved · ${dayShort[day.isoDay - 1]} ${slotLabel(slot.start_hour, slot.end_hour)}`)
    } catch (err) {
      setHours(before)
        ...
        e.id === event.id
          ? {
              ...
              responses: joined
                ...
                : [...e.responses.filter((r) => r.user_id !== userId), { user_id: userId, status: 'coming' as const }],
            }
          : e,
      ),
    )
  ...
  async function fillUsual() {
    ...
    try {
      ...
      await week.reload()
      toast(n > 0 ? `Filled in ${n} hour${n === 1 ? '' : 's'} from your usual week` : 'Nothing new to add')
    } catch (err) {
      setError(friendlyError(err))
    } finally {
      setBusy(false)
  ...
  const usualButton = (
    <Button variant="secondary" size="sm" onClick={() => void saveUsual()} disabled={busy || myHourCount === 0} className="w-full lg:w-auto">
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z" />
        <path d="M17 21v-8H7v8M7 3v5h8" />
      </svg>
      Use this week as my usual week
    </Button>
  )
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-4 lg:flex-row lg:gap-6">
      <div className="flex min-h-0 min-w-0 flex-1 flex-col gap-4">
        ...
        {showBanner && (
          <div className="flex flex-col gap-3 rounded-2xl border-[1.5px] border-green bg-green-soft px-4 py-3.5 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-col gap-0.5 text-green-ink">
              <div className="text-sm font-extrabold">You haven't answered for week {weekNo} yet</div>
              <div className="text-[13px]">
                {hasUsual ? 'Tap the evenings you can, or start from your usual week and adjust.' : 'Tap the evenings you can play. You can pick more than one per day.'}
              </div>
            </div>
            {hasUsual && (
              <Button size="sm" onClick={() => void fillUsual()} disabled={busy} className="h-[38px]">
        ...
        <SessionsList events={events} types={week.types} members={members} userId={userId} onToggleJoin={(e) => void toggleJoin(e)} hint="tap to join or leave" />

        {!showBanner && <p className="px-1 text-sm text-muted">Tap the evenings you can play. You can pick more than one per day.</p>}

        {/* Desktop: 7 columns */}
        <div className="hidden min-h-0 flex-1 grid-cols-7 content-start gap-2.5 overflow-auto rounded-[20px] bg-surface p-5 shadow-card lg:grid">
          {days.map((day) => {
            ...
            return (
              <div key={day.key} className="flex min-w-0 flex-col gap-2">
                <DayHeader day={day} />
                {daySlots.length === 0 && <div className="rounded-xl border-[1.5px] border-dashed border-[#eceae5] py-3 text-center text-[11px] text-faint">no slots</div>}
        ...
        <div className="flex flex-col gap-2.5 lg:hidden">
          {days.map((day) => {
            const daySlots = slots.filter((s) => s.day_type === (day.isWeekend ? 'weekend' : 'weekday'))
            return (
              <div key={day.key} className="flex items-center gap-2.5 rounded-2xl bg-surface py-3.5 pl-3.5 pr-3 shadow-card">
                ...
                {daySlots.length === 0 ? (
                  <span className="text-sm text-faint">No time slots for this day.</span>
                ) : (
                  <div className="grid flex-1 gap-2" style={{ gridTemplateColumns: `repeat(${Math.min(daySlots.length, 3)}, minmax(0, 1fr))` }}>
                    {daySlots.map((slot) => renderSlot(day, slot, daySlots, true))}
                  </div>
                )}
              </div>
            )
      ...
      <aside className="hidden w-[300px] shrink-0 flex-col gap-4 lg:flex lg:min-h-0">
        <div className="flex flex-col gap-3.5 rounded-[20px] bg-surface p-5 shadow-card">
          <h2 className="text-[15px] font-extrabold">Saved as you go</h2>
          <p className="text-[13px] leading-relaxed text-muted">Every tap is saved right away, and teammates see it live. Nothing to submit.</p>
          {usualButton}
        </div>
          ...
          <ul className="flex min-h-0 flex-col gap-2.5 overflow-y-auto">
            {members.map((m) => (
              <li key={m.user_id} className="flex items-center gap-2.5">
                <Avatar name={m.profile?.display_name ?? '?'} url={m.profile?.avatar_url} size={30} />
                <div className="flex min-w-0 flex-col">
                  <span className="truncate text-[13px] font-bold">{m.profile?.display_name ?? 'Unknown'}</span>
                  <span className="text-[11px] text-muted">
                    {roleLabel[m.role]} · {m.user_id === userId ? 'you' : answered.has(m.user_id) ? 'answered' : 'not answered'}
                  </span>
                </div>
              </li>
            ))}
  ...
  function renderSlot(day: WeekDay, slot: TeamSlot, daySlots: TeamSlot[], compact = false) {
    ...
    return (
      <button
        ...
        className={`flex h-12 flex-col items-center justify-center gap-1 rounded-xl border-[1.5px] text-[12px] font-bold leading-none tracking-tight transition disabled:opacity-60 ${
          on ? 'border-green bg-green-soft text-green-ink' : 'border-line bg-surface text-ink hover:border-faint'
...
function DayHeader({ day }: { day: WeekDay }) {
  return (
    <div className="flex flex-col items-center gap-0.5 pb-1">
      <span className={`text-xs font-bold ${day.isToday ? 'text-green-ink' : 'text-muted'}`}>{dayShort[day.isoDay - 1]}</span>
      {day.isToday ? (
        <span className="flex h-[26px] w-[26px] items-center justify-center rounded-full bg-ink text-[13px] font-extrabold text-white">{format(day.date, 'd')}</span>
      ) : (
        <span className="text-[15px] font-extrabold">{format(day.date, 'd')}</span>
      )}
    </div>
  )
}
