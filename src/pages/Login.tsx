      ...
      <div className="mx-auto flex w-full max-w-[1120px] flex-1 flex-col items-center gap-12 px-6 py-14 lg:flex-row lg:items-center lg:gap-16 lg:py-16">
        <div className="flex w-full max-w-[520px] flex-col gap-7">
          ...
          <div className="flex flex-col gap-5">
            ...
            <p className="max-w-[46ch] text-[15px] leading-relaxed text-muted">
              Then let the bot do the talking: it posts the week in your server, keeps it up to date, and tells the people it concerns when something is added, moved or cancelled. Join with one button, right in Discord.
            </p>
            <p className="max-w-[46ch] text-[15px] leading-relaxed text-muted">
...
const CARDS: { key: string; label: string; card: ReactNode; steps: [string, string][] }[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      ['Join from Discord', 'Join or Can’t with one button. The website and the post always agree.'],
    ],
  },
  {
    key: 'updates',
...
const peekPeople = ['Daniel', 'Sander', 'Mathias', 'Jonas', 'Emil'].map((n, i) => ({
  user_id: `p${i}`,
  profile: { user_id: `p${i}`, display_name: n, avatar_url: null, discord_name: n, custom_name: false, timezone: null, dm_opt_out: false },
}))
