...
export default function Login() {
  ...
  const [error, setError] = useState<string | null>(
    authError ? `Sign-in failed: ${authError}` : null,
  );
  ...
  return (
    <main className="flex min-h-dvh flex-col">
      <div className="mx-auto flex w-full max-w-[1120px] flex-1 flex-col items-center gap-12 px-6 py-14 lg:flex-row lg:items-center lg:gap-16 lg:py-16">
        <div className="flex w-full max-w-[520px] flex-col gap-7">
          ...
          <div className="flex flex-col gap-5">
            <h1 className="text-[34px] font-extrabold leading-[1.1] tracking-tight sm:text-[50px]">
              When can you play this week?
            </h1>
            <p className="max-w-[46ch] text-[15px] leading-relaxed text-muted">
              Sick of juggling messages and polls on Discord? Gatherapp.gg keeps
              track of your group’s availability. Plan activities like scrims, matches,
              hangouts or whatever. No signups, no ads, no tracking.
            </p>
            <p className="max-w-[46ch] text-[15px] leading-relaxed text-muted">
              Then let the bot do the talking: it posts a weekplan in your server,
              keeps it up to date, and updates users directly when
              something is added, moved or cancelled.
            </p>
            <p className="max-w-[46ch] text-[15px] leading-relaxed text-muted">
          ...
          <div className="flex flex-col gap-3">
            ...
            <Button
              onClick={handleLogin}
              className="h-[60px] w-full gap-3 text-[16px] sm:w-fit sm:px-9"
            >
              <ChatIcon />
      ...
      <footer className="px-6 pb-8 text-center text-[12px] text-faint">
...
const CARDS: {
...
}[] = [
  {
    ...
    steps: [
      ...
      [
        "Book it",
        "Set up the weekplan and share it on Discord. With link or bot.",
      ],
    ],
  },
  {
    ...
    steps: [
      ...
      [
        "Display the weekplan automatically",
        "The bot posts and updates the weekplan in a channel of your choice.",
      ],
  ...
  },
  {
    ...
    steps: [
      [
        ...
        "Bot keeps track of weekplan and updates users directly.",
      ],
      [
        "Channel or DM",
        "Updates can be sent to a channel and/or as a DM to each player.",
      ],
      [
        "Automatic updates",
        "Updates are posted automatically when the weekplan changes.",
      ],
    ],
  },
];
          ...
          return (
            <div
              ...
              style={{
                transform:
                  behind === 0
                    ? "none"
                    : `translate(-${behind * 12}px, -${behind * 12}px)`,
            ...
            >
              ...
              <div
                ...
                style={{
                  opacity: behind === 0 ? 0 : behind === 1 ? 0.45 : 0.7,
                }}
              />
            </div>
          );
...
const peekPeople = ["Daniel", "Sander", "Mathias", "Jonas", "Emil"].map(
  (n, i) => ({
    user_id: `p${i}`,
    profile: {
      user_id: `p${i}`,
    ...
    <div className="flex h-full w-full max-w-[520px] flex-col gap-3 rounded-[22px] bg-surface p-5 shadow-card">
      ...
      <div className="grid grid-cols-5 gap-1.5">
        {peekDays.map((d) => (
          <span
            key={d}
            className="pb-0.5 text-center text-[11px] font-bold text-muted"
          >
            {d}
          </span>
        ))}
            ...
            return (
              <div
                ...
                className={`flex flex-col items-center justify-center gap-1 rounded-[10px] border-[1.5px] py-2 ${
                  all
                    ? "border-green-dim bg-green-soft"
                    : "border-line-soft bg-bg"
                }`}
              >
                <span
                  className={`text-[10px] font-extrabold tabular-nums ${all ? "text-green-ink" : "text-muted"}`}
                >
                  {peekHours[r]}
                </span>
      ...
      >
        <div className="flex w-9 shrink-0 flex-col items-center leading-none">
          <span
            ...
            style={{ color: "var(--act-coral-sub)" }}
          >
            Wed
          </span>
          <span
            className="text-[17px] font-extrabold"
            style={{ color: "var(--act-coral-ink)" }}
          >
            10
          </span>
        </div>
      ...
      >
        <div className="flex w-9 shrink-0 flex-col items-center leading-none">
          <span
            className="text-[9px] font-bold uppercase tracking-[0.1em]"
            style={{ color: "var(--act-yellow-sub)" }}
          >
            Tue
          </span>
          <span
            className="text-[17px] font-extrabold"
            style={{ color: "var(--act-yellow-ink)" }}
          >
            9
          </span>
        </div>
        ...
        <AvatarStack
          people={peekPeople.slice(0, 3)}
          size={26}
          ring="var(--act-yellow-soft)"
        />
      </div>
    </div>
  );
...
function DiscordFrame({ children }: { children: ReactNode }) {
  return (
    <div
      className="flex h-full w-full max-w-[520px] flex-col gap-2.5 rounded-[22px] p-5 shadow-card"
      style={{ background: DC.panel, color: DC.text }}
    >
      <div className="flex items-center gap-3">
        <span
          className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-white"
          style={{ background: DC.green }}
        >
          <svg
            width="18"
            height="18"
        ...
        <div className="flex items-center gap-2 leading-none">
          ...
          <span
            className="rounded-[4px] px-1.5 py-[3px] text-[9px] font-extrabold uppercase tracking-wide text-white"
...
function DiscordCard({
  ...
  return (
    <div
      className="flex flex-col gap-2 rounded-[8px] border-l-4 px-3.5 py-3"
      style={{ background: DC.card, borderColor: accent }}
    >
      {children}
    </div>
  );
...
function DiscordButtons({ yes, dim }: { yes: string; dim?: boolean }) {
  return (
    <div className="flex gap-2 pt-1">
      <span
        className="rounded-[4px] px-3.5 py-1.5 text-[12px] font-bold text-white"
        style={{ background: dim ? DC.grey : DC.green }}
      >
        {yes}
      </span>
      <span
        className="rounded-[4px] px-3.5 py-1.5 text-[12px] font-bold text-white"
        style={{ background: DC.grey }}
      >
        Can’t
      </span>
    </div>
  ...
  return (
    <DiscordFrame>
      <div className="flex flex-col gap-2">
        <DiscordCard accent={DC.yellow}>
          <span className="text-[15px] font-extrabold text-white">
            Moved · Scrim vs Foxes
          </span>
          <span className="flex flex-wrap gap-1.5">
            {["Daniel", "Sander", "Mathias"].map((n) => (
              <span
                key={n}
                className="rounded-[3px] px-1 text-[12px] font-semibold"
                style={{ background: DC.chip, color: DC.chipText }}
              >
                @{n}
              </span>
            ))}
        ...
        <span className="pl-1 text-[11px]" style={{ color: DC.faint }}>
          In the updates channel, as a DM to each player, or both. The team
          picks.
        </span>
      </div>
    </DiscordFrame>
