...
// Discord allows 40 components in a V2 message. Each session is five (separator,
// text, row, two buttons); header, footer and the ping line take another five.
const MAX_EVENTS = 6
...
export function buildWeekMessage(week: BotWeek, opts: { ping: string | null; link: PingLink | null; appUrl: string }): Record<string, unknown> {
  ...
  if (opts.ping) blocks.push({ type: 10, content: opts.ping })
  blocks.push({ type: 10, content: `## ${week.team.name} · Week ${nr}\n-# ${weekRangeLabel(week.week_start)}` })

  const events = week.events.slice(0, MAX_EVENTS)
  if (events.length === 0) {
    blocks.push({ type: 14, divider: true, spacing: 1 })
    blocks.push({ type: 10, content: 'Nothing booked yet.' })
  }
  for (const e of events) {
    ...
    const who = e.people.length ? `${e.people.map(person).join(' ')}  ·  ${e.people.length} in` : 'nobody yet'
    blocks.push({ type: 14, divider: true, spacing: 1 })
    blocks.push({ type: 10, content: `**${dayName(e.date)}** <t:${start}:t> – <t:${end}:t>\n${title}\n-# ${who}` })
    blocks.push({
      type: 1,
      components: [
  ...
  if (week.events.length > MAX_EVENTS) {
    blocks.push({ type: 14, divider: true, spacing: 1 })
    blocks.push({ type: 10, content: `-# +${week.events.length - MAX_EVENTS} more in the app` })
  }
  blocks.push({ type: 14, divider: true, spacing: 1 })
  blocks.push({ type: 10, content: `-# Updated <t:${unix(new Date())}:R>  ·  [Open in Gather](${url})` })

  const accent = events[0] ? (ACCENT[events[0].color] ?? GATHER_GREEN) : GATHER_GREEN
